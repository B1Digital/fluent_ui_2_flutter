...
void main() {
  ...
  testWidgets('DeclarativeChartBasicExample', (tester) async {
    ...
    await expectReactParity(
      ...
      // Measured 0.000% — not one of 327,332 unmasked pixels differs, in any
      // zone. Pinned at 0: the floor check admits nothing else.
      //
      // History: 37.309% before the y domain fix; 32.408% while
      // `FluentAreaChart` had no `selectedLegends`, so series b, c and d
      // painted at fill opacity 0.7 where upstream dims them to 0.1
      // (`DeclarativeChart.tsx:598-603`); 0.032% until the legend swatches
      // snapped to whole device pixels, which took the partial end columns
      // off the two dimmed swatches' 1px outlines.
      maxMismatch: 0,
    );
  });
  ...
  testWidgets('VegaDeclarativeChartDefault', (tester) async {
    ...
    await expectReactParity(
      ...
      // Measured 0.175% — 349 of 199,098 unmasked pixels, in every zone, and
      // none of them is the chart. All 349 are columns x 0-1, rows 2-350: the
      // right border and rounded top-right corner of the story's schema
      // textarea, which sits against the chart's left edge inside the capture
      // clip. Capture scaffolding, not text, so nothing masks it; closing it
      // takes a clip decision and a re-capture, not a port change. Plot,
      // markers, gridlines and the circular legend swatches all match.
      //
      // History: 12.515% while every kind sized its cell from
      // `spec['height']` — upstream ignores `height: 400` for a `point` mark
      // (`transformVegaLiteToScatterChartProps`, `:3191-3231`, returns no
      // dimensions; `ScatterChart.tsx:137-139`), so `_fitParentContainer`
      // (`CartesianChart.tsx:514-519`) falls back to 350 and takes the 40px
      // legend strip out of it (Oracle B: svg 600x310, legend root 580x32).
      // 1.370% before three fixes: the series order (`Object.keys` hoists the
      // integer-like keys "3" and "4", now `jsObjectKeys` in
      // `internal/vega/js_value.dart`); the chart's own left-aligned circular
      // legend (`ScatterChart.tsx:279`) in place of a lifted centred strip,
      // since `:494` lifts the legend for the concat branch only; and the
      // harness sizing below. 0.224% until the legend swatches snapped to
      // whole device pixels. Dropping the lifted strip costs the single-spec
      // legend round-trip through `onSchemaChange`; `vega_declarative_chart.dart`
      // records that gap.
      //
      // The harness: Oracle B puts the root box at y 339.5, height 350, so the
      // screenshot spans 351 rows for a 350px box and Chromium rounds the
      // origin UP to the second of them. Sizing the chart to the PNG would
      // both stretch it by a row and start it one row high.
    ...
    // box would fail the comparison above long before it reached the pin.
